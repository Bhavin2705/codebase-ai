...
def find_matching_brace(code: str, start_index: int) -> int:
    """
    Finds index of matching closing brace '}' starting from start_index.
    Correctly accounts for strings, template literals, and single/block comments.
    """
    n = len(code)
...
class PythonASTSymbolVisitor(ast.NodeVisitor):
    """Extracts classes, functions, and async functions from Python AST."""
...
class CodeParserService:
    """
    Syntactic code parser extracting classes, functions, routes, models, and interfaces.
    """

    @staticmethod
    def _chunk_file(file_path: str, code_content: str) -> List[Dict[str, Any]]:
    ...
    def parse_file(self, file_path: str, code_content: str, language: str) -> List[Dict[str, Any]]:
        # For large files (>50KB) or minified lines (>1000 chars), produce deterministic line chunks
    ...
    def _parse_javascript_typescript(self, file_path: str, code_content: str) -> List[Dict[str, Any]]:
        ...
        # 1. Express / Router route declarations: router.get('/path', ...)
        ...
        # 6. Arrow functions: const UserCard = (...) => { ... }
