...
import { MapPin, Send, RotateCcw, Trash2 } from 'lucide-react';
import LoadingStopwatch from './LoadingStopwatch';
import ThoughtProcessWidget from './ThoughtProcessWidget';

function parseInlineMarkdown(lineStr) {
...
function renderFormattedAnswer(text, onSelectCitation) {
  if (!text) return null;

  // Handle markdown citations: [Label](cite:filePath#Lstart-Lend)
  ...
  while ((match = citeRegex.exec(text)) !== null) {
    const [_, labelText, filePath, startLine, endLine] = match;
  ...
  return segments.map((seg, idx) => {
    ...
    lines.forEach((rawLine, lineIdx) => {
      ...
      if (line.startsWith('- ') || line.startsWith('* ')) {
        renderedElements.push(
          <li key={lineIdx} style={{ marginLeft: '16px', marginBottom: '3px' }}>
    ...
    return <span key={`text-block-${idx}`}>{renderedElements}</span>;
  });
}
          ...
          <div key={msg.id} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div className="message-bubble user">
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px' }}>
                <h2 className="user-question-header" style={{ margin: 0, flex: 1 }}>
                  {msg.question}
                </h2>
              ...
              {msg.isPending && !msg.answer ? (
                ...
                <>
                  ...
                  <div className="answer-body">
                    {renderFormattedAnswer(msg.answer, onSelectCitation)}
                    {msg.isPending && (
                      <span
