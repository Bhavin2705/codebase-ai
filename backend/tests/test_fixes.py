...
async def test_reindexing_already_indexed_commit():
    ...
    with pytest.MonkeyPatch.context() as m:
        ...
        m.setattr(indexing_service.git, "get_commit_sha", lambda dir: "abcdef1234567890abcdef1234567890abcdef12")
    ...
    async with AsyncSessionLocal() as db:
        ...
        assert res_repo.current_version_id == ver_id

@pytest.mark.anyio
async def test_failed_indexing_preserves_current_version_id():
    async with AsyncSessionLocal() as db:
        repo_id = uuid.uuid4()
        v1_id = uuid.uuid4()
        now = datetime.now(timezone.utc)
    ...
    async with AsyncSessionLocal() as db:
        ...
        assert res_repo is not None
        assert res_repo.current_version_id == v1_id
        assert res_repo.status == "ready"

@pytest.mark.anyio
async def test_retrieval_uses_current_version_id():
    async with AsyncSessionLocal() as db:
        repo_id = uuid.uuid4()
        v1_id = uuid.uuid4()
        v2_id = uuid.uuid4()
        ...
        repo = Repository(
            id=repo_id,
            name="test-org/version-retrieval-repo",
            github_url=f"https://github.com/test-org/version-retrieval-repo-{repo_id.hex[:6]}",
            language="Python",
            status="ready",
            current_version_id=v2_id,
            indexed_at=now
        )
        v1 = RepositoryVersion(id=v1_id, repository_id=repo_id, commit_sha="1111111111111111111111111111111111111111", status="active")
        v2 = RepositoryVersion(id=v2_id, repository_id=repo_id, commit_sha="2222222222222222222222222222222222222222", status="active")

        f1_id = uuid.uuid4()
        f1 = FileModel(id=f1_id, repository_version_id=v1_id, path="old_file.py", language="python", content="def old_fn(): pass")
        ...
        f2_id = uuid.uuid4()
        f2 = FileModel(id=f2_id, repository_version_id=v2_id, path="new_file.py", language="python", content="def new_fn(): pass")
        ...
        contexts, _ = await retrieval_service.retrieve_contexts("how does function work?", str(repo_id), db)
...
async def test_chat_persistence_using_repository_name_identifier():
    ...
    async with AsyncSessionLocal() as db:
        r = Repository(id=repo_uuid, name=repo_name, github_url=f"https://github.com/{repo_name}", language="Python", status="ready", current_version_id=v_uuid)
        v = RepositoryVersion(id=v_uuid, repository_id=repo_uuid, commit_sha="3333333333333333333333333333333333333333", status="active")
        db.add(r)
        db.add(v)
        await db.commit()
